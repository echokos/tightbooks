...
/**
 * Returns an ApiFetcher configured with auth headers and Accept: application/json+table
 * for report endpoints that return table format. Use with fetchBalanceSheetTable, etc.
 */
export function useReportsApiFetcher() {
  const token = useAuthToken();
  const organizationId = useAuthOrganizationId();
  const currentLocale = getCookie('locale');
  ...
  }, [token, organizationId, currentLocale]);
}
